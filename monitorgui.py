...
class ConfigWindow(Frame):
    ...
    def layout_monitor(self):
        ...
        self.bar_canvas_width = (
            300  # Total width allocated for the bars within the canvas
        )
        ...
        self.canvas_monitor_height = (
            self.bar_height_monitor + self.bar_vertical_spacing
        ) * 8

        # Create the Canvas widget
        self.progress_canvas = Canvas(
            self,
            bg="lightgray",  # Background color for the canvas area
        ...
        )  # Color of the canvas border
        # Place the canvas in column 1, spanning 8 rows
        self.progress_canvas.grid(
            column=1, row=1, rowspan=8, sticky="nsew", padx=5, pady=2
        )
        ...
        for j in range(8):
            ...
            bar_rect = self.progress_canvas.create_rectangle(
                ...
                y2,
                fill="blue",
                outline="",  # Default initial color, no outline for the bar itself
            )
    ...
    def layout_settings(self):
        ...
        for i, section_name in enumerate(self.section_names):
            ...
            selected_settings[i].trace_add(
                "write", menu_item_selected
    ...
    def setconfig(self):
        ...
        try:
            ...
            if config is not None:
                rate = config.get("devices", {}).get("samplerate")
            ...
            if rate is not None:
                config["devices"]["samplerate"] = (
                    rate  # Reapply original sample rate if available
                )
    ...
    def update(self):
        ...
        if state == camilladsp.ProcessingState.RUNNING:
            ...
            for i in range(8):
                ...
                scaled_value = max(
                    0, min(100, scaled_value)
                )  # Clamp value between 0 and 100

                bar_id = self.monitor_bars_canvas_ids[
                    i
                ]  # Get the canvas ID for the current bar
                ...
                color = self._map_monitor_amplitude_to_color(
                    scaled_value, max_value=100
                )
...
class SpectrumAnalyzer(Frame):
    def __init__(self, master=None, bar_height_max=150, **kwargs):
        ...
        total_bars_width = (
            self.num_bars * (self.bar_width + self.bar_spacing) - self.bar_spacing
        )
        self.canvas_width = total_bars_width

        # Create the Canvas for the spectrum analyzer display
        self.canvas = Canvas(
            self, bg="black", height=self.canvas_height, width=self.canvas_width
        )
        ...
        for i in range(self.num_bars):
            ...
            y1_bar_bottom = (
                self.bar_max_height + 10
            )  # Baseline for the bottom of the bars
            x2 = x1 + self.bar_width
            y2_bar_bottom = self.bar_max_height + 10

            # Create an empty rectangle (initially at baseline)
            bar_rect = self.canvas.create_rectangle(
                x1, y1_bar_bottom, x2, y2_bar_bottom, fill="green", outline=""
            )
            self.bars.append(bar_rect)

            text_x = x1 + self.bar_width / 2  # Center text below the bar
            text_y = (
                self.bar_max_height + self.text_area_height + 5
    ...
    def update(self):
        ...
        if state == camilladsp.ProcessingState.RUNNING:
            ...
            for i, amp in enumerate(spectrum):
                ...
                bar_actual_height = max(
                    1, bar_actual_height
                )  # Ensure bar has minimum height of 1 pixel
                ...
                self.canvas.coords(
                    self.bars[i],
                    x1,
                    (self.bar_max_height + 10) - bar_actual_height,
                    x2,
                    self.bar_max_height + 10,
                )

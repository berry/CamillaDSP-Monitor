...
cmd = ["./camilladsp", "setting.yml", "-p",
       "1234", '-g-20', '-l', 'warn', '-w']
scmd = ["./camilladsp", "spectrum.yml", "-p", "5678", '-l', 'warn', '-w']
proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
sproc = subprocess.Popen(scmd, stdout=subprocess.PIPE,
...
def print_output(msg, vol, rate, values, spectrum, mute):
    ...
    stdscr.addstr(
        0, 0, f"Volume {volume_string(vol)} {mute_string}  Rate: {rate}\n")
    line = 1
    for v in volume:
        for s in source:
            for c in channel:
...
while True:
    ...
    try:
        ...
        if state == camilladsp.ProcessingState.RUNNING:
            values = [c.get_capture_signal_rms(), c.get_playback_signal_rms(
            ), c.get_capture_signal_peak(), c.get_playback_signal_peak()]
            values = sum(values, [])
